 ...
 */
const retrieveLatestAccessToken = async (refreshTokenId: bigint) => {
  // Retrieve the access token from the database
...
const validateAccessToken = async (userId: string, token: string) => {
  ...
  // Check if this is the latest access token generated by the refresh token
  const latestAccessToken = await retrieveLatestAccessToken(accessToken.refresh_token);

  if (latestAccessToken?.token !== accessToken.token) {
