...
const revokeAccessToken = async (token: string) => {
  ...
  return PrismaClient.access_tokens.update({
    where: {
      token: token,
    },
    data: {
 ...
 */
const validateToken = async <T extends refresh_tokens>(userId: string, token: T | null) => {
...
const validateRefreshToken = async (userId: string, token: string) => {
  ...
  return validateToken(userId, refreshToken);
};
...
const validateAccessToken = async (userId: string, token: string) => {
  ...
  return validateToken(userId, accessToken);
};
